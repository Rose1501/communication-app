...
class StudentGroupsScreen extends StatefulWidget {
  ...
  const StudentGroupsScreen({super.key, required this.studentId , required this.studentname});
...
class _StudentGroupsScreenState extends State<StudentGroupsScreen> {
  ...
  void _loadStudentGroups({bool forceRefresh = false}) {
    if (!forceRefresh) {
      // 🔍 محاولة جلب البيانات من التخزين المحلي أولاً
      final cachedData = _cacheService.getStudentGroups(widget.studentId);
      if (cachedData != null && cachedData.isNotEmpty) {
        setState(() {
          _cachedCourses = cachedData;
        });
        print('✅ تم تحميل ${_cachedCourses.length} مادة من التخزين المحلي');
        return;
      }
    }
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<SubjectiveBloc>().add(LoadStudentGroupsEvent(widget.studentId));
    });
  }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<SubjectiveBloc, SubjectiveState>(
      listener: (context, state) {
        if (state is StudentGroupsLoadSuccess) {
          setState(() {
            _isLoading = false;
            _cachedCourses = state.courses;
          });
          // 💾 حفظ البيانات في التخزين المحلي
          _cacheService.cacheStudentGroups(widget.studentId, state.courses);
      ...
      builder: (context, state) {
        ...
        return RefreshIndicator(
          ...
          onRefresh: () async {
            _loadStudentGroups(forceRefresh: true);
          },
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () => _loadStudentGroups(forceRefresh: true),
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.school_outlined, size: 80, color: ColorsApp.grey),
          const SizedBox(height: 16),
          Text(
          ...
          Text(
            'سيتم عرض المقررات هنا عندما تسجل فيها',
            style: font16Grey,
            textAlign: TextAlign.center,
          ),
        ],
